...
RUN cd /usr/local && git clone https://github.com/HW-RajKD/HelloWorld.git

# Step-9 : Build the project
RUN cd /usr/local/HelloWorld && /usr/local/maven/bin/mvn -Dmaven.test.skip=true package

# Step-10 : Deploy the war in tomcat
RUN rm -rf ${CATALINA_HOME}/webapps/*
RUN cp /usr/local/HelloWorld/target/SampleSkeleton.war /usr/local/tomcat/apache-tomcat-${TOMCAT_MINOR_VERSION}/webapps/
